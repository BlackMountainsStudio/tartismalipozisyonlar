...
interface HakemlerClientProps {
  initialReferees: RefereeListItem[];
}

export default function HakemlerClient({ initialReferees }: HakemlerClientProps) {
  ...
  const filteredReferees =
    roleFilter === "all"
      ? initialReferees
      : initialReferees.filter((r) => r.role === roleFilter);
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <div className="mb-10 text-center">
        <div className="mb-4 flex justify-center">
          <div className="rounded-2xl bg-red-500/10 p-4">
            <UserRound className="h-10 w-10 text-red-500" aria-hidden="true" />
          </div>
        </div>
      ...
      <div className="mb-8">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <label htmlFor="radar-referee-filter" className="text-xs text-zinc-500">Hakem filtresi (radar):</label>
          <select
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        ...
        <div className="flex rounded-xl border border-zinc-700 bg-zinc-800/80 p-0.5" role="group" aria-label="Hakem rolü filtresi">
          {FILTER_TABS.map((tab) => (
            <button
              key={tab.value}
              type="button"
      ...
      {initialReferees.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-800 py-20 text-center">
          <UserRound className="mb-4 h-12 w-12 text-zinc-700" aria-hidden="true" />
