...
export default function YorumcuDetayPage({
  params,
}: {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <button
        onClick={() => router.push("/yorumcular")}
        ...
        <div className="mb-8 rounded-xl border border-zinc-800 bg-zinc-900/50 p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {commentator.expertise.map((item, i) => (
              <span
                key={i}
                className="rounded-xl border border-zinc-700 bg-zinc-800/80 px-4 py-2 text-sm font-medium text-zinc-300"
              >
                {item}
              </span>
            ))}
                ...
                >
                    ...
                    <Link
                      href={
                        op.incident?.match
                          ? incidentUrl(
                              buildMatchSlug(op.incident.match),
                              buildIncidentSlug({
                                id: op.incident.id,
                                minute: op.incident.minute,
                                description: op.incident.description,
                              })
                            )
                          : `/incidents/${op.incident.id}`
                      }
