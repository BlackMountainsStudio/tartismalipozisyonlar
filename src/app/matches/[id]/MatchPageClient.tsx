...
export interface IncidentForPage {
  id: string;
  type: string;
  minute: number | null;
  description: string;
...
interface MatchPageClientProps {
  match: MatchForPage;
  incidents: IncidentForPage[];
}

export default function MatchPageClient({ match, incidents }: MatchPageClientProps) {
  ...
  const pointsFor = (inc: IncidentForPage) => getIncidentImpactPoints(inc.type);
  const impactHomeAgainst = wrongIncidents.filter((i) => i.against === match.homeTeam).reduce((s, i) => s + pointsFor(i), 0);
  const impactHomeFavor = wrongIncidents.filter((i) => i.inFavorOf === match.homeTeam).reduce((s, i) => s + pointsFor(i), 0);
  const impactAwayAgainst = wrongIncidents.filter((i) => i.against === match.awayTeam).reduce((s, i) => s + pointsFor(i), 0);
  const impactAwayFavor = wrongIncidents.filter((i) => i.inFavorOf === match.awayTeam).reduce((s, i) => s + pointsFor(i), 0);
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-10 rounded-2xl border border-zinc-800 bg-gradient-to-br from-zinc-900 to-zinc-950 p-8 text-center">
        <div className="mb-4 flex items-center justify-center gap-2">
          ...
          <span className="text-sm font-medium text-red-400">{match.league}</span>
        </div>
          ...
          <div className="mt-4 flex flex-col items-center gap-2">
            ...
            {wrongCount > 0 && (impactHomeAgainst > 0 || impactHomeFavor > 0 || impactAwayAgainst > 0 || impactAwayFavor > 0) && (
              <div className="flex flex-wrap items-center justify-center gap-4 text-xs">
                <Link href="/rehber" className="text-zinc-500 hover:text-amber-400" title="Puanlama rehberi">
                  Maç etkisi (puan)
                </Link>
                {(impactHomeAgainst > 0 || impactHomeFavor > 0) && (
                  <span className="text-zinc-400">
                    {match.homeTeam}:{" "}
                    {impactHomeFavor > 0 && <span className="text-emerald-400">+{impactHomeFavor} lehine</span>}
                    {impactHomeFavor > 0 && impactHomeAgainst > 0 && " · "}
                    {impactHomeAgainst > 0 && <span className="text-red-400">-{impactHomeAgainst} aleyhine</span>}
                  </span>
                )}
                {(impactAwayAgainst > 0 || impactAwayFavor > 0) && (
                  <span className="text-zinc-400">
                    {match.awayTeam}:{" "}
                    {impactAwayFavor > 0 && <span className="text-emerald-400">+{impactAwayFavor} lehine</span>}
                    {impactAwayFavor > 0 && impactAwayAgainst > 0 && " · "}
                    {impactAwayAgainst > 0 && <span className="text-red-400">-{impactAwayAgainst} aleyhine</span>}
                  </span>
                )}
              </div>
            )}
      ...
      {incidents.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-800 py-16 text-center">
          ...
          <p className="text-lg text-zinc-400">Bu maç için tartışmalı pozisyon bulunamadı</p>
          <p className="mt-1 text-sm text-zinc-500">Hakem kararlarında önemli bir tartışma tespit edilmedi</p>
        </div>
      ) : (
