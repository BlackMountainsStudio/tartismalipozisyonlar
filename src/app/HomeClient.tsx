...
import { Shield, TrendingUp, AlertTriangle, Search, ArrowUpDown, Users, Check } from "lucide-react";
...
export default function HomeClient({ initialMatches }: HomeClientProps) {
  ...
  const seasonMatches =
    initialMatches.filter((m) => m.league === "Süper Lig 2025-26").length > 0
      ? initialMatches.filter((m) => m.league === "Süper Lig 2025-26")
      : initialMatches;
  ...
  const groupedMatches = filteredMatches.reduce<Record<number, MatchForHome[]>>((groups, match) => {
      ...
      <section className="border-b border-zinc-800 bg-gradient-to-b from-zinc-900 to-zinc-950">
        <div className="mx-auto max-w-7xl px-4 py-16 text-center sm:px-6 sm:py-24">
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-3 sm:gap-6">
            <StatCard
              icon={<TrendingUp className="h-5 w-5 text-emerald-400" aria-hidden="true" />}
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <label htmlFor="match-search" className="sr-only">Maç ara</label>
            <input
              id="match-search"
              type="text"
              value={search}
        ...
        <div className="mb-6 flex flex-col gap-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 sm:flex-row sm:flex-wrap sm:items-center">
          ...
          <div className="hidden h-4 w-px bg-zinc-700 sm:block" aria-hidden="true" />
        ...
        {filteredMatches.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-800 py-20 text-center">
            <Shield className="mb-4 h-12 w-12 text-zinc-700" aria-hidden="true" />
